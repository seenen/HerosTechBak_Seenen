...
public class CameraScissor : MonoBehaviour 
{
	...
	private static Rect scissorRect = new Rect (0,0,1,1);
	private static Camera camera1 = null;
	
	public static void SetScissor(float _left, float _top, float _width, float _height)
	{
		float left 	 = _left/Screen.width;
		float top 	 = (Screen.height - _top - _height)/Screen.height;
		float width  = _width/Screen.width;
		float height = _height/Screen.height;
		
		scissorRect = new Rect (left,top,width,height);
		
		if(camera1 != null)
			SetScissorRect(camera1, scissorRect);
		else
			Debuger.LogError("Scissor is NULL !");
	}
