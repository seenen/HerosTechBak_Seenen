    ...
    static void CreateAssetBundle_Character(PLATFORM platform)
    {
        ...
        foreach (Object obj in Selection.GetFiltered(typeof(Object), SelectionMode.DeepAssets))
        {
            ...
            //			if(false)
            {
                ...
                {
                    result = BuildPipeline.BuildAssetBundle(prefab, null, path, BuildAssetBundleOptions.CollectDependencies, BuildTarget.iPhone);
                }
            ...
            //			if(false)
            {
                ...
                foreach (SkinnedMeshRenderer smr in characterFBX.GetComponentsInChildren<SkinnedMeshRenderer>(true))
                {
                    ...
                    {
                        result = BuildPipeline.BuildAssetBundle(null, toinclude.ToArray(), path, BuildAssetBundleOptions.CollectDependencies, BuildTarget.iPhone);
                    }
                    else if (platform == PLATFORM.Android)
                    {
