...
public class Assembler : MonoBehaviour
{
    ...
    private void UpdateWing(GameObject root, string wingName, string wingEleName)
    {
        ...
        else
        {
            ...
            mWingState = wing.animation["n_run"];
            if (mWingState != null)
                wing.animation.Play(mWingState.name);
        }
    }
    ...
    private void CreateAnimation(GameObject root, string character)
    {
        ...
        if (root.animation.GetClipCount() == 0)
        {
            ...
            {
                root.animation.AddClip(anim.Value, anim.Key);
            }
        }

        root.animation.cullingType = AnimationCullingType.BasedOnRenderers;
    }
