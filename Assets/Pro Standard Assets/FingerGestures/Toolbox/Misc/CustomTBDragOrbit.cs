...
public class CustomTBDragOrbit : MonoBehaviour
{
    public enum PanMode
    {
        Disabled,
	...
	void Awake()
	{
		camera.transform.position = cameraFar.transform.position;
		camera.transform.localEulerAngles = cameraFar.transform.localEulerAngles;
	}
    ...
    void FingerGestures_OnDragMove( Vector2 fingerPos, Vector2 delta )
    {
        ...
        if( target )
        {
            ...
            IdealYaw += delta.x * yawSensitivity * 0.02f * direction;
            IdealPitch += delta.y * pitchSensitivity * 0.02f * direction;
        }
    }
    ...
    void Apply()
    {
        ...
        if( smoothMotion )
        {
			...
			transform.RotateAround(new Vector3(1, 0, 0), (pitch - tempPitch)/360.0f);
			transform.RotateAround(new Vector3(0, 1, 0), (yaw - tempYaw)/360.0f);
        }
        else
        {
		...
		{
//        	camera.gameObject.transform.position = ( target.position + panOffset ) - (distance) * camera.gameObject.transform.forward;
			
			camera.transform.position = cameraFar.transform.position + 
									(cameraNear.transform.position - cameraFar.transform.position) * (distance-minDistance)/(maxDistance-minDistance);
			
			Vector3 farAngle = cameraFar.transform.localEulerAngles;
			if(farAngle.x < 180)
				farAngle.x += 360;
			
			camera.transform.localEulerAngles = farAngle +
									(cameraNear.transform.localEulerAngles - farAngle) * (distance-minDistance)/(maxDistance-minDistance);
